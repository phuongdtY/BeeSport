...
//    Page<SanPham> getAll();
    ...
    @Query("SELECT NEW com.poly.application.model.response.SanPhamMoiNhatResponse(sp.id, sp.ten, MIN(cps.giaTien), MAX(cps.giaTien)) FROM SanPham sp JOIN ChiTietSanPham cps ON sp.id = cps.sanPham.id WHERE cps.trangThai = 'ACTIVE' GROUP BY sp.id, sp.ten ORDER BY MAX(cps.ngayTao) DESC")
